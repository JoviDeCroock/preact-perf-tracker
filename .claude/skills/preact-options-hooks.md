...
A component is mounting if there is no previous props snapshot / no `alternate`. The simplest approach: track whether you've seen the component instance before (e.g., via a WeakMap or a custom property on the component).
...
options.__r = (vnode) => {
  if (vnode.__c) startTimes.set(vnode.__c, performance.now());
};

options.diffed = (vnode) => {
  const c = vnode.__c;
  if (c && startTimes.has(c)) {
    const elapsed = performance.now() - startTimes.get(c);
    console.log(getDisplayName(vnode), elapsed.toFixed(2) + 'ms');
    startTimes.delete(c);
  }
};
```
